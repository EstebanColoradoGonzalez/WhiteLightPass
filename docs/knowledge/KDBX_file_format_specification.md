...
- A UUID is a unique identifier consisting of 16 bytes (128 bits). In this specification, we list the bytes of a UUID in hexadecimal form, without a prefix. For example, 31C1F2E6BF714350BE5805216AFC5AFF = (0x31, 0xC1, 0xF2, 0xE6, 0xBF, 0x71, 0x43, 0x50, 0xBE, 0x58, 0x05, 0x21, 0x6A, 0xFC, 0x5A, 0xFF). In the [XML document](https://keepass.info/help/kb/kdbx.html#xml), UUIDs are stored in Base64 encoding.
- In this specification, we use the symbol '‖' for the concatenation of two byte sequences, with implicit conversion of the arguments to byte sequences, without padding. Example:(0xAB, 0xCD, 0xEF) ‖ 0x12345678 ‖ "Abc" = (0xAB, 0xCD, 0xEF, 0x78, 0x56, 0x34, 0x12, 0x41, 0x62, 0x63).
...
1. [Header](https://keepass.info/help/kb/kdbx.html#header).
2. SHA-256 hash of the header.
3. HMAC-SHA-256 hash of the header.
4. In [HMAC-protected block stream](https://keepass.info/help/kb/kdbx.html#hbs):
    - Encrypted:
        - Compressed (optional):
            1. [Inner header](https://keepass.info/help/kb/kdbx.html#iheader).
            2. [XML document](https://keepass.info/help/kb/kdbx.html#xml).
...
The HMAC-SHA-256 hash of the header can be used for checking the integrity and the authenticity of the header. The correct master key is required for verifying the HMAC. See the section '[Computation of Keys](https://keepass.info/help/kb/kdbx.html#keys)' for details about the computation of the HMAC key.
...
• If the minor version of the file is greater than the one that the application supports, the application may try to load the file, ignoring any unknown items. Certain data may be lost in this case, thus showing a confirmation/warning is recommended.For KDBX 4.1, the format version value is 0x00040001. |
...
• `D6038A2B8B6F4CB5A524339A31DBB59A`: ChaCha20 (RFC 8439).Plugins can provide more encryption algorithms. |
| Compression algorithm | 3 | UInt32 | 0 = no compression, 1 = GZip. |
| Master salt/seed (⟳) | 4 | Byte[32] | Salt/seed for [computing the keys](https://keepass.info/help/kb/kdbx.html#keys). |
...
| KDF parameters | 11 | [Variant dictionary](https://keepass.info/help/kb/kdbx.html#vardict) | Parameters for the key derivation function (KDF). See below. |
| Public custom data | 12 | [Variant dictionary](https://keepass.info/help/kb/kdbx.html#vardict) | Custom data of plugins/ports. The name of an item should be unique, e.g. "PluginName_ItemName". In this header field, only data that must be readable without decryption should be stored (e.g. data by a key provider plugin required for decryption). All other custom data should be stored in the encrypted [XML document](https://keepass.info/help/kb/kdbx.html#xml) (elements `//Meta/CustomData`, `//Group/CustomData` and `//Entry/CustomData`). |

The IDs 1, 5, 6, 8, 9 and 10 were used in previous versions of the KDBX file format.

**Key derivation function (KDF) parameters.** The KDF parameters are stored in a [variant dictionary](https://keepass.info/help/kb/kdbx.html#vardict). The following parameters are supported:
...
• `9E298B1956DB4773B23DFC3EC6F0A1E6`: Argon2id.Plugins can provide more KDFs. |
| **AES-KDF:** |  |  |
| `S` (⟳) | Byte[32] | Salt/seed. |
| `R` | UInt64 | Rounds. |
| **Argon2:** |  |  |
...
1. Format version, as UInt16.The current version is 1.0, i.e. 0x0100.
...
1. Let  be the SHA-256 hash of the concatenation of the components of the master key that the user has provided (each optional, in the following order):
    
    R
    
    1. SHA-256 hash of the [master password](https://keepass.info/help/base/keys.html#password) (encoded using UTF-8).
    2. Key stored in a [key file](https://keepass.info/help/base/keys.html#keyfiles).
    3. Key provided by a [key provider](https://keepass.info/help/v2_dev/plg_keyprov.html) plugin.
    4. Key protected using the [Windows user account](https://keepass.info/help/base/keys.html#winuser) (DPAPI).
2. Let  be the result of transforming  using a [key derivation function](https://keepass.info/help/base/security.html#secdictprotect). The function and parameters for it are stored in the [header](https://keepass.info/help/kb/kdbx.html#header).
...
- If the encryption algorithm needs a 256-bit key (such as AES-256 and ChaCha20), the key is:SHA-256( ‖ ).If the encryption algorithm needs a key smaller than 256 bits, the key consists of the first bytes of SHA-256( ‖ ).
...
- The key for the HMAC-SHA-256 hash of the header is:SHA-512(0xFFFFFFFFFFFFFFFF ‖ SHA-512( ‖  ‖ 0x01)).
    
    S
    
    T
    
- The key for the HMAC-SHA-256 hash of the th block (zero-based index, type UInt64) of the [HMAC-protected block stream](https://keepass.info/help/kb/kdbx.html#hbs) is:SHA-512( ‖ SHA-512( ‖  ‖ 0x01)).
...
The inner encryption key is stored in the [inner header](https://keepass.info/help/kb/kdbx.html#iheader) and does not depend on the master key. For details, see the description of the [inner encryption](https://keepass.info/help/kb/kdbx.html#ienc).
...
HMAC-SHA-256(i ‖ s ‖ M) ‖ s ‖ M.

For computing the HMAC-SHA-256 hash, a key is required. See the section '[Computation of Keys](https://keepass.info/help/kb/kdbx.html#keys)' for details about the computation of the key.
...
Let K be the inner encryption key stored in the [inner header](https://keepass.info/help/kb/kdbx.html#iheader). The parameters for the inner encryption algorithm are derived as follows:

- Salsa20.  should consist of 32 bytes. The key for Salsa20 is SHA-256(), and the nonce is (0xE8, 0x30, 0x09, 0x4B, 0x97, 0x20, 0x5D, 0x2A).
    
    K
    
    K
    
- ChaCha20.  should consist of 64 bytes. Compute  := SHA-512(). The key for ChaCha20 is ([0], ..., [31]), and the nonce is ([32], ..., [43]).
...
[KDBX_XML.xsd](https://keepass.info/help/download/KDBX_XML.xsd)
